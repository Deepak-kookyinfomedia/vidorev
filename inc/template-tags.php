...
if(!function_exists('vidorev_main_logo_retina')):
	function vidorev_main_logo_retina(){
		...
		if($main_logo!=''){
		?>
			<img src="<?php echo esc_url($main_logo);?>" alt="<?php echo esc_attr($alt);?>" class="main-logo" width="auto" height="auto">
		<?php
		}else{
		?>
			<img src="<?php echo esc_url($main_logo_sample);?>" alt="<?php echo esc_attr($alt);?>" class="main-logo" width="auto" height="auto">
		<?php
		}
		
		if($main_logo_mobile!=''){
		?>
			<img src="<?php echo esc_url($main_logo_mobile);?>" alt="<?php echo esc_attr($alt);?>" class="main-logo-mobile" width="auto" height="auto">
		<?php
		}else{
		?>
			<img src="<?php echo esc_url($main_logo_mobile_sample);?>" alt="<?php echo esc_attr($alt);?>" class="main-logo-mobile" width="auto" height="auto">
		<?php
		}
		
		if($sticky_logo!=''){
		?>
			<img src="<?php echo esc_url($sticky_logo);?>" alt="<?php echo esc_attr($alt);?>" class="sticky-logo" width="auto" height="auto">
		<?php	
		}else{
		?>
			<img src="<?php echo esc_url($sticky_logo_sample);?>" alt="<?php echo esc_attr($alt);?>" class="sticky-logo" width="auto" height="auto">
		<?php
		}	
	...
	function vidorev_social_accounts_listing($arg){	
		...
		<ul class="social-block <?php echo esc_attr($arg[0]);?>">
			...
			if(vidorev_get_redux_option('login_user_btn', 'on', 'switch')=='on'){
				...
				?>
					<li class="login-elm">
						<?php if(class_exists('BuddyPress') && (class_exists('Youzer') || class_exists('Youzify'))){?>
	...
	function vidorev_nav_breadcrumbs($args){
		...
		if (is_home() || is_front_page()) {
			...
			if ( get_query_var('paged') ) {
				if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() || (is_single() && get_post_type() == 'post' && get_post_format() == 'video') || (is_single() && ( is_singular('vid_playlist') || is_singular('vid_channel') || is_singular('vid_actor') || is_singular('vid_director') )) ) echo ' (';
...
if ( ! function_exists( 'vidorev_posted_on' ) ) :
	...
	function vidorev_posted_on($args, $type = NULL) {
		...
		if(isset($args[0]) && $args[0] === 'author' && $postMetaControl[0] === 'author' && is_numeric($author_id) && $author_id>0){
			...
			if(function_exists('youzify_is_account_verification_enabled') && youzify_is_account_verification_enabled()){
				$is_account_verified = get_user_meta( $author_id, 'youzify_account_verified', true );
				if($is_account_verified == 'on'){
					$verify_user_html = ' <span class="vd-user-verify fa fa-check-circle" aria-hidden="true"></span>';
...
if( ! function_exists( 'vidorev_thumbnail' ) ):
	function vidorev_thumbnail($size = 'thumbnail', $ratio='class-16x9', $wrapper = 1, $post_id = NULL, $ratio_case = NULL, $imdb_elms = 0){
		...
		$playlist_counting = '';
		$count_playlists_tx = esc_html__('Videos', 'vidorev');
		...
		if($post_type == 'vid_playlist' && defined('PLAYLIST_PM_PREFIX')){
			...
			if(is_array($playlist_videos)){
				...
				$count_playlists_dm = count($playlist_videos);
				if($count_playlists_dm == 1){
					$count_playlists_tx = esc_html__('Video', 'vidorev');
				}						
				
				$playlist_counting.= '<span class="playlist-count font-size-12 meta-font"><i class="fa fa-play-circle" aria-hidden="true"></i><span>'.esc_attr(apply_filters('vidorev_number_format', $count_playlists_dm)).' '.$count_playlists_tx.'</span></span>';
			}
			...
			if(is_array($series_group)){
				...
				$playlist_counting = '<span class="playlist-count font-size-12 meta-font"><i class="fa fa-play-circle" aria-hidden="true"></i><span>'.esc_attr(apply_filters('vidorev_number_format', $series_videos)).' '.$count_playlists_tx.'</span></span>';
			}
...
if ( ! function_exists( 'vidorev_single_post_title' ) ) :
	function vidorev_single_post_title( $position = 'basic', $extra_class = '' ){
		...
		<?php	
			...
			$pos_paged 		= strpos($current_url , '/page/');
			
			if($pos_paged!==FALSE){
				$current_url 	= trailingslashit(substr($current_url, 0, $pos_paged));
			}
					...
					<div class="video-player-content video-player-control">
						...
						<div class="player-3rdparty player-3rdparty-control <?php if($player_library=='videojs' || $player_library=='flow') echo esc_attr('player-loaded');?>">
							...
							if($nextVideo > 0){
								...
								if(isset($beeteam368_single_vm_video_multi_links) && is_array($beeteam368_single_vm_video_multi_links) && isset($beeteam368_single_vm_video_multi_links_index) && is_numeric($beeteam368_single_vm_video_multi_links_index)){
									if($beeteam368_single_vm_video_multi_links_index == count($beeteam368_single_vm_video_multi_links)){
										$next_multi = 1;
									}else{
										$next_multi = $beeteam368_single_vm_video_multi_links_index + 1;
									}
								}
...
if ( !function_exists( 'vidorev_popular_videos_footer' ) ):
	function vidorev_popular_videos_footer() {
		...
		global $beeteam368_page_template_id;
		
		if(is_page() || (isset($beeteam368_page_template_id) && is_numeric($beeteam368_page_template_id))){
			
			if(isset($beeteam368_page_template_id) && is_numeric($beeteam368_page_template_id)){
				$page_id = $beeteam368_page_template_id;
			}else{
				$page_id = get_the_ID();
			}			
				...
				if(!is_numeric($user_rating_multi_sets) || !class_exists('YasrMultiSetData')){
					...
					if(is_numeric($multiset_average) && $multiset_average > 0){
						if($rt_nbov){
							...
							if(!method_exists('YasrMultiSetData', 'returnMultisetContent')){
								return '';
							}
							
							$refl = new ReflectionMethod('YasrMultiSetData', 'returnMultisetContent');
					
							if(!$refl->isStatic() || !$refl->isPublic()){
								return '';
							}
							
							$multiset_content = YasrMultiSetData::returnMultisetContent($post_id, $user_rating_multi_sets, true);
